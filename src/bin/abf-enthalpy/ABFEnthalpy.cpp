...
// print enthalpy
    if(PrintH() == false) {
        ES_ERROR("unable to print derivatives of dST to output");
        return(false);
    }
    return(true);
}
...
bool CABFEnthalpy::PrintH(void)
{
    ...
    // write block delimiter - required by GNUPlot
        ...
        double value = Accumulator.GetValue(ibin,EABF_H_VALUE);
        double sigma = Accumulator.GetValue(ibin,EABF_H_SIGMA);
