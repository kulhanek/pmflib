...
#include "ABFEnergyOptGPRHyprms.hpp"
#include <iomanip>
...
MAIN_ENTRY(CABFEnergyOptGPRHyprms)

//==============================================================================
//------------------------------------------------------------------------------
//==============================================================================

CABFEnergyOptGPRHyprms::CABFEnergyOptGPRHyprms(void)
{
    NCVs = 0;
}
...
int CABFEnergyOptGPRHyprms::Init(int argc,char* argv[])
{
...
// attach verbose stream to cout and set desired verbosity level
    ...
    vout << endl;
    vout << "# ==============================================================================" << endl;
    vout << "# abf-energy-optgprhyprms (PMFLib utility)  started at " << StartTime.GetSDateAndTime() << endl;
...
bool CABFEnergyOptGPRHyprms::Run(void)
{
    State = 1;
...
void CABFEnergyOptGPRHyprms::InitOptimizer(void)
{
    vout << endl;
...
void CABFEnergyOptGPRHyprms::DecodeEList(const CSmallString& spec, std::vector<bool>& elist,const CSmallString& optionname)
{
    string          sspecen(spec);
    vector<string>  slist;
...
void CABFEnergyOptGPRHyprms::DecodeVList(const CSmallString& spec, CSimpleVector<double>& vlist,const CSmallString& optionname,double defv)
{
    string          sspecen(spec);
    vector<string>  slist;
...
bool CABFEnergyOptGPRHyprms::Optimize(void)
{
    bool result = true;
...
bool CABFEnergyOptGPRHyprms::ResetOpt(int& numofreset)
{
    numofreset++;
...
double CABFEnergyOptGPRHyprms::GetGNorm(void)
{
    double gnorm = 0.0;
    int ind = 0;
...
void CABFEnergyOptGPRHyprms::Test(void)
{
    vout << endl;
...
void CABFEnergyOptGPRHyprms::CalcHessian(void)
{
    vout << endl;
...
void CABFEnergyOptGPRHyprms::PrintGradientSummary(void)
{
    vout << endl;
...
void CABFEnergyOptGPRHyprms::ShowGPRStat(void)
{
    vout << endl;
...
void CABFEnergyOptGPRHyprms::RunGPRAnalytical(void)
{
// setup parameters
    ScatterHyprms(Hyprms);
...
void CABFEnergyOptGPRHyprms::RunGPRNumerical(void)
{
    logTarget = 0;
    HyprmsGrd.SetZero();
    for(size_t i=0; i < Accumulators.size(); i++){
...
double CABFEnergyOptGPRHyprms::RunGPRNumerical(CABFAccumulatorP accu,CSimpleVector<double>& der)
{
    CSimpleVector<double>   tmp_prms;
    tmp_prms.CreateVector(NumOfOptPrms);
...
void CABFEnergyOptGPRHyprms::ScatterHyprms(CSimpleVector<double>& hyprsm)
{
    // update parameters
...
double CABFEnergyOptGPRHyprms::GetTarget(CABFIntegratorGPR& gpr,CABFAccumulatorP accu)
{
    CEnergySurface fes;
    fes.Allocate(accu.get());
...
void CABFEnergyOptGPRHyprms::WriteResults(int istep)
{
...
void CABFEnergyOptGPRHyprms::PrintSampledStat(void)
{
    for(size_t i=0; i < Accumulators.size(); i++){
        CABFAccumulatorP accu = Accumulators[i];
        vout << format("** ABFAccumulator #%02d ...")%(i+1) << endl;
...
bool CABFEnergyOptGPRHyprms::WriteHyperPrms(FILE* p_fout)
{
    if( p_fout == NULL ){
        return(false);
    }
...
void CABFEnergyOptGPRHyprms::LoadGPRHyprms(void)
{
    ifstream fin;
...
void CABFEnergyOptGPRHyprms::Finalize(void)
{
    ...
    vout << endl;
    vout << "# ==============================================================================" << endl;
    vout << "# abf-energy-optgprhyprms terminated at " << dt.GetSDateAndTime() << ". Total time: " << dur.GetSTimeAndDay() << endl;
    vout << "# ==============================================================================" << endl;
