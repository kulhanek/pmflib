...
#include "ABFEnthalpyOptGPRHyprms.hpp"
#include <iomanip>
...
MAIN_ENTRY(CABFEnthalpyOptGPRHyprms)

//==============================================================================
//------------------------------------------------------------------------------
//==============================================================================

CABFEnthalpyOptGPRHyprms::CABFEnthalpyOptGPRHyprms(void)
{
    NCVs = 0;
}
...
int CABFEnthalpyOptGPRHyprms::Init(int argc,char* argv[])
{
...
bool CABFEnthalpyOptGPRHyprms::Run(void)
{
    State = 1;
...
void CABFEnthalpyOptGPRHyprms::InitOptimizer(void)
{
    vout << endl;
...
void CABFEnthalpyOptGPRHyprms::DecodeEList(const CSmallString& spec, std::vector<bool>& elist,const CSmallString& optionname)
{
    string          sspecen(spec);
    vector<string>  slist;
...
void CABFEnthalpyOptGPRHyprms::DecodeVList(const CSmallString& spec, CSimpleVector<double>& vlist,const CSmallString& optionname,double defv)
{
    string          sspecen(spec);
    vector<string>  slist;
...
bool CABFEnthalpyOptGPRHyprms::Optimize(void)
{
    bool result = true;
...
bool CABFEnthalpyOptGPRHyprms::ResetOpt(int& numofreset)
{
    numofreset++;
...
double CABFEnthalpyOptGPRHyprms::GetGNorm(void)
{
    double gnorm = 0.0;
    int ind = 0;
...
void CABFEnthalpyOptGPRHyprms::Test(void)
{
    vout << endl;
...
void CABFEnthalpyOptGPRHyprms::CalcHessian(void)
{
    vout << endl;
...
void CABFEnthalpyOptGPRHyprms::PrintGradientSummary(void)
{
    vout << endl;
...
void CABFEnthalpyOptGPRHyprms::ShowGPRStat(void)
{
    vout << endl;
...
void CABFEnthalpyOptGPRHyprms::RunGPRAnalytical(void)
{
// setup parameters
    ScatterHyprms(Hyprms);
...
void CABFEnthalpyOptGPRHyprms::RunGPRNumerical(void)
{
    logTarget = 0;
    HyprmsGrd.SetZero();
    for(size_t i=0; i < Accumulators.size(); i++){
...
double CABFEnthalpyOptGPRHyprms::RunGPRNumerical(CABFAccumulatorP accu,CSimpleVector<double>& der)
{
    CSimpleVector<double>   tmp_prms;
    tmp_prms.CreateVector(NumOfOptPrms);
...
void CABFEnthalpyOptGPRHyprms::ScatterHyprms(CSimpleVector<double>& hyprsm)
{
    // update parameters
...
double CABFEnthalpyOptGPRHyprms::GetTarget(CABFEnthalpyGPR& gpr,CABFAccumulatorP accu)
{
    CEnergySurface fes;
    fes.Allocate(accu.get());
...
void CABFEnthalpyOptGPRHyprms::WriteResults(int istep)
{
...
void CABFEnthalpyOptGPRHyprms::PrintSampledStat(void)
{
    for(size_t i=0; i < Accumulators.size(); i++){
        CABFAccumulatorP accu = Accumulators[i];
        vout << format("** ABFAccumulator #%02d ...")%(i+1) << endl;
...
bool CABFEnthalpyOptGPRHyprms::WriteHyperPrms(FILE* p_fout)
{
    if( p_fout == NULL ){
        return(false);
    }
...
void CABFEnthalpyOptGPRHyprms::LoadGPRHyprms(void)
{
    ifstream fin;
...
void CABFEnthalpyOptGPRHyprms::Finalize(void)
{
