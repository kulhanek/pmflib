...
#include "ABFOptEntGPRHyprms.hpp"
#include <iomanip>
...
MAIN_ENTRY(CABFOptEntGPRHyprms)

//==============================================================================
//------------------------------------------------------------------------------
//==============================================================================

CABFOptEntGPRHyprms::CABFOptEntGPRHyprms(void)
{
    NCVs = 0;
}
...
int CABFOptEntGPRHyprms::Init(int argc,char* argv[])
{
...
bool CABFOptEntGPRHyprms::Run(void)
{
    State = 1;
...
void CABFOptEntGPRHyprms::InitOptimizer(void)
{
    vout << endl;
...
void CABFOptEntGPRHyprms::DecodeEList(const CSmallString& spec, std::vector<bool>& elist,const CSmallString& optionname)
{
    string          sspecen(spec);
    vector<string>  slist;
...
void CABFOptEntGPRHyprms::DecodeVList(const CSmallString& spec, CSimpleVector<double>& vlist,const CSmallString& optionname,double defv)
{
    string          sspecen(spec);
    vector<string>  slist;
...
bool CABFOptEntGPRHyprms::Optimize(void)
{
    bool result = true;
...
bool CABFOptEntGPRHyprms::ResetOpt(int& numofreset)
{
    numofreset++;
...
double CABFOptEntGPRHyprms::GetGNorm(void)
{
    double gnorm = 0.0;
    int ind = 0;
...
void CABFOptEntGPRHyprms::Test(void)
{
    vout << endl;
...
void CABFOptEntGPRHyprms::CalcHessian(void)
{
    vout << endl;
...
void CABFOptEntGPRHyprms::PrintGradientSummary(void)
{
    vout << endl;
...
void CABFOptEntGPRHyprms::ShowGPRStat(void)
{
    vout << endl;
...
void CABFOptEntGPRHyprms::RunGPRAnalytical(void)
{
// setup parameters
    ScatterHyprms(Hyprms);
...
void CABFOptEntGPRHyprms::RunGPRNumerical(void)
{
    logTarget = 0;
    HyprmsGrd.SetZero();
    for(size_t i=0; i < Accumulators.size(); i++){
...
double CABFOptEntGPRHyprms::RunGPRNumerical(CABFAccumulatorP accu,CSimpleVector<double>& der)
{
    CSimpleVector<double>   tmp_prms;
    tmp_prms.CreateVector(NumOfOptPrms);
...
void CABFOptEntGPRHyprms::ScatterHyprms(CSimpleVector<double>& hyprsm)
{
    // update parameters
...
double CABFOptEntGPRHyprms::GetTarget(CABFEnthalpyGPR& gpr,CABFAccumulatorP accu)
{
    CEnergySurface fes;
    fes.Allocate(accu.get());
...
void CABFOptEntGPRHyprms::WriteResults(int istep)
{
...
void CABFOptEntGPRHyprms::PrintSampledStat(void)
{
    for(size_t i=0; i < Accumulators.size(); i++){
        CABFAccumulatorP accu = Accumulators[i];
        vout << format("** ABFAccumulator #%02d ...")%(i+1) << endl;
...
bool CABFOptEntGPRHyprms::WriteHyperPrms(FILE* p_fout)
{
    if( p_fout == NULL ){
        return(false);
    }
...
void CABFOptEntGPRHyprms::LoadGPRHyprms(void)
{
    ifstream fin;
...
void CABFOptEntGPRHyprms::Finalize(void)
{
