...
// attach verbose stream to cout and set desired verbosity level
    ...
    vout << endl;
    vout << "# ==============================================================================" << endl;
    vout << "# abf-energy (PMFLib utility)  started at " << StartTime.GetSDateAndTime() << endl;
...
void CABFEnergyIntegrate::LoadGPRHyprms(CABFIntegratorGPR& gpr)
{
    ifstream fin;
    fin.open(Options.GetOptLoadHyprms());
    if( ! fin ){
...
int CABFEnergyIntegrate::GlueingFES(int factor)
{
    ...
    for(int ibin=0; ibin < Accumulator.GetNumOfBins(); ibin++) {
        if( Accumulator.GetNumOfSamples(ibin) != 0 ) continue; // skip glued or sampled bins
        ...
        for(int j=0; j < ndir; j++){
            ...
            if( tbin >= 0 ){
                if( factor == 1 ){
                    ...
                    if( Accumulator.GetNumOfSamples(tbin) == -factor ){
                        Accumulator.SetNumberOfABFSamples(ibin,-(factor+1));
                        glued++;
                        break;
                    }
...
void CABFEnergyIntegrate::Finalize(void)
{
    ...
    vout << endl;
    vout << "# ==============================================================================" << endl;
    vout << "# abf-energy terminated at " << dt.GetSDateAndTime() << ". Total time: " << dur.GetSTimeAndDay() << endl;
    vout << "# ==============================================================================" << endl;
