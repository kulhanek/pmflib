...
void CABPProcessor::ExchangeData(void)
{
    int client_id = -1;
    ...
    CRegClient* p_client = ABPServer.RegClients.FindClient(client_id);
    if(p_client == NULL) {
    ...
    // exchange data
    try{
        ABPServer.ABPAccumulator.ExchangeDataWithClient(p_client);
