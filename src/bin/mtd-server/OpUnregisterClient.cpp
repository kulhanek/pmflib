...
void CMTDProcessor::UnregisterClient(void)
{
    int client_id = -1;
...
//  and unregister it from list
    MTDServer.RegClients.UnregisterClient(client_id);

// automatically shutdown server if last client is unregistered
    if(MTDServer.RegClients.GetNumberOfActiveRegistration() == 0) {
        if(MTDServer.DoNotShutdown == false) MTDServer.TerminateServer();
    }
}
