...
subroutine abf_control_read_abf(prm_fin)
    ...
    call pmf_ctrl_check_integer_in_range('ABF','ftds_ekin_src',ftds_ekin_src,1,6)
...
!===============================================================================
! Subroutine:  abf_control_read_shake_cvs
!===============================================================================

subroutine abf_control_read_shake_cvs(prm_fin)
    ...
    write(PMF_OUT,*)
    call pmf_utils_heading(PMF_OUT,'{ABF-SHAKE}',':')
    write(PMF_OUT,*)

    ! get name of group
    if( fabfshakedef(1:1) .eq. '{' ) then
        grpname = fabfshakedef(2:len_trim(fabfshakedef)-1)
         write(PMF_OUT,110) trim(grpname)
        ! open goup with name from abfdef
        if( .not. prmfile_open_group(prm_fin,trim(grpname)) ) then
            return
        end if
        call abf_control_read_shake_cvs_from_group(prm_fin)
    else
        write(PMF_OUT,120) trim(fabfshakedef)

        call prmfile_init(locprmfile)

        if( .not. prmfile_read(locprmfile,fabfshakedef) ) then
            call pmf_utils_exit(PMF_OUT,1,'[ABF] Unable to load file: ' // trim(fabfshakedef) // '!')
        end if

        call abf_control_read_shake_cvs_from_group(locprmfile)
...
110 format('Collective variables (ABF/SHAKE) are read from group: ',A)
120 format('Collective variables (ABF/SHAKE) are read from file : ',A)

end subroutine abf_control_read_shake_cvs

!===============================================================================
! Subroutine:  abf_control_read_shake_cvs_from_group
!===============================================================================

subroutine abf_control_read_shake_cvs_from_group(prm_fin)
    ...
    ! count number of sections in group
    NumOfABFSHAKECVs = prmfile_count_group(prm_fin)

    if( NumOfABFSHAKECVs .le. 0 ) then
        ! on SHAKE CV in current or specified group
        write(PMF_OUT,100)
        return
    end if

    write(PMF_OUT,110) NumOfABFSHAKECVs

    ! allocate constraint list ----------------------------------------------------
    allocate(ABFSHAKECVList(NumOfABFSHAKECVs), stat = alloc_failed)
    ...
    do i=1,NumOfABFSHAKECVs
        call abf_cvs_reset_cv(ABFSHAKECVList(i))
    end do
    ...
    do while(eresult)
        ...
        ABFSHAKECVList(i)%cvindx = cv_common_find_cv(cvname)
        ABFSHAKECVList(i)%cv     => CVList(ABFSHAKECVList(i)%cvindx)%cv
    ...
    ! check if there is CV overlap
    do i=1,NumOfABFSHAKECVs
        do j=i+1,NumOfABFSHAKECVs
            if( ABFSHAKECVList(i)%cvindx .eq. ABFSHAKECVList(j)%cvindx ) then
                call pmf_utils_exit(PMF_OUT,1, &
                     '[ABF] Two different ABF collective variables share the same general collective variable!')
            end if
        end do
...
100 format('>>> INFO: No ABF/SHAKE CVs are defined. Skipping ...')
110 format('Number of ABF/SHAKE collective variables         : ',I4)
130 format('== Reading collective variable #',I4.4)
140 format('   Collective variable name : ',a)

end subroutine abf_control_read_shake_cvs_from_group
