...
subroutine abf_core_main
    ...
    select case(fmode)
        ...
        case default
            call pmf_utils_exit(PMF_OUT,1,'[ABF] Not implemented code mode!')
    end select
...
subroutine abf_core_force()
    ...
    ! apply force filters
    call abf_core_force_filters(cvaluehist1,la)
...
!===============================================================================
! subroutine:  abf_core_force_filters
!===============================================================================

subroutine abf_core_force_filters(values,gfx)
    ...
    ! apply max force filter
    do i=1,NumOfABFCVs
        if( ABFCVList(i)%maxforce .eq. 0 ) cycle
        if( abs(gfx(i)) .ge. ABFCVList(i)%maxforce ) then
            ! if the force is greater than maxforce cut it to zero
            gfx(i) = 0.0d0
        end if
    end do
    ...
    ! apply switch
    do i=1,NumOfABFCVs
        if( values(i) .lt. ABFCVList(i)%min_value ) cycle
        if( values(i) .gt. ABFCVList(i)%max_value ) cycle
        sc = 1.0d0
        if( (values(i) - ABFCVList(i)%min_value) .lt. ABFCVList(i)%switch ) then
            v = values(i) - ABFCVList(i)%min_value
            v = ABFCVList(i)%switch - v
            v = v / ABFCVList(i)%switch
            sc = 1.0d0 - 10.0d0*v**3 + 15.0d0*v**4 - 6.0d0*v**5
        end if

        if( (ABFCVList(i)%max_value - values(i)) .lt. ABFCVList(i)%switch ) then
            v = ABFCVList(i)%max_value - values(i)
            v = ABFCVList(i)%switch - v
            v = v / ABFCVList(i)%switch
            sc = 1.0d0 - 10.0d0*v**3 + 15.0d0*v**4 - 6.0d0*v**5
        end if
...
end subroutine abf_core_force_filters
