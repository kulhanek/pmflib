...
subroutine abf_core_main
    ...
    select case(fmode)
        ...
        case(3)
            ! simplified ABF algorithm
            call abf_core_force_3pA_noshake
        case default
...
subroutine abf_core_force_3pA()
    ...
    integer                :: i,j,k,m
    integer                :: ci,ki
    real(PMFDP)            :: v,v1,v2,fp,fs,etot,epot,erst,ekin
    ! --------------------------------------------------------------------------
...
! ABF part
    if( fstep .ge. hist_len ) then
        ...
        ! write(7894,*) fstep, fp, fs, 0.5d0*(v1+v2)*ifdtx, ekinhist(hist_len-1)

        ! total ABF force
        pxi1(:) = pxi0(:) - micfhist(:,hist_len-1)  ! unbiased estimate
        ...
        call abf_accu_add_data_online(cvhist(:,hist_len-1),pxi1,pxi0,epothist(hist_len-1),ersthist(hist_len-1),etot)
    end if
...
subroutine abf_core_force_3pA_noshake()
        ...
        do i=1,NumOfABFCVs
            ...
            do j=1,NumOfLAtoms
                do m=1,3
                    ...
                    ! velocity part
                    v1 = v1 + (zdhist(m,j,i,hist_len-0)-zdhist(m,j,i,hist_len-1)) * (vhist(m,j,hist_len-0) - vshist(m,j,hist_len-1))
                    v2 = v2 + (zdhist(m,j,i,hist_len-1)-zdhist(m,j,i,hist_len-2)) * (vhist(m,j,hist_len-1) - vshist(m,j,hist_len-2))
                end do
            end do
...
end subroutine abf_core_force_3pA_noshake
