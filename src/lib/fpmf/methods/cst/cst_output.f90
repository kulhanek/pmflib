...
! write data
    write(CST_OUT,170,advance='NO') fstep,nsamples,pmf_unit_get_rvalue(TimeUnit,ftime),fsiter

! go to energy units - calculate metric tensor correction
    if( nsamples .ne. 0 ) then
    ...
    do i=1,nitems
        ...
        if( nsamples .gt. 0 ) then
            lam      = lambda(i)
            alam     = mlambda(i)
            lams     = sqrt(m2lambda(i)/real(nsamples,PMFDP))
            alams    = lams / sqrt(real(nsamples,PMFDP))
        else
            lam     = 0.0d0
            alam    = 0.0d0
            lams    = 0.0d0
...
! output formats -----------------------------
170 format(1X,I8,1X,F8.0,1X,F11.1,1X,I2)
...
! write data
    write(CST_OUT,170,advance='NO') fnstlim,nsamples,pmf_unit_get_rvalue(TimeUnit,ftime)
...
170 format('#',I8,1X,F8.0,1X,F11.1)
180 format(1X,E15.8)
190 format(1X,E15.8)
200 format(1X,E15.8,1X,E15.8,1X,E15.8)
