...
type, extends(CVType) :: CVTypeMUL

    integer :: left_cv
    integer :: right_cv

    contains
        procedure :: load_cv        => load_mul
        procedure :: calculate_cv   => calculate_mul
end type CVTypeMUL
...
subroutine load_mul(cv_item,prm_fin)
    ...
    ! this CV does not have any groups or atoms
    cv_item%ngrps = 0
    ...
    ! set CV unit
    cv_item%unit = pmf_unit_div_units(CVList(cv_item%left_cv)%cv%unit,CVList(cv_item%left_cv)%cv%unit)
...
end subroutine load_mul

!===============================================================================
! Subroutine:  calculate_mul
!===============================================================================

subroutine calculate_mul(cv_item,x,ctx)
