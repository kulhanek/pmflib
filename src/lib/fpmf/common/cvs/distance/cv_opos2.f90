...
type, extends(CVType) :: CVTypeOPOS2

    type(XYZFILE_TYPE)  :: xyz_str_a
    integer :: direction

    contains
        procedure :: load_cv        => load_opos2
        procedure :: calculate_cv   => calculate_opos2
end type CVTypeOPOS2
...
!===============================================================================
! Subroutine:  load_opos2
!===============================================================================

subroutine load_opos2(cv_item,prm_fin)
...
end subroutine load_opos2

!===============================================================================
! Subroutine:  calculate_opos2
!===============================================================================

subroutine calculate_opos2(cv_item,x,ctx)
    ...
    real(PMFDP)         :: o(3),tmp1(3)
    ! --------------------------------------------------------------------------
    real(PMFDP)         :: a_u(3,3)
    real(PMFDP)         :: a_o(3),a_b(3)
    ...
    if( info .ne. 0 ) then
        call pmf_utils_exit(PMF_OUT,1,'Unable to diagonalize matrix in calculate_opos2!')
    end if
    ...
    select case(cv_item%direction)
        case(1)
            ! x
            ctx%CVsValues(cv_item%idx) = px(1)*u(1,1) + px(2)*u(2,1) + px(3)*u(3,1)
        case(2)
            ! y
            ctx%CVsValues(cv_item%idx) = px(1)*u(1,2) + px(2)*u(2,2) + px(3)*u(3,2)
        case(3)
            ! z
            ctx%CVsValues(cv_item%idx) = px(1)*u(1,3) + px(2)*u(2,3) + px(3)*u(3,3)
        case default
            call pmf_utils_exit(PMF_OUT,1,'Unrecognized value for direction in calculate_opos2!')
    end select
...
end subroutine calculate_opos2
