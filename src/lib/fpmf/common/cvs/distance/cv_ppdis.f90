...
!    Copyright (C) 2015 Jakub Planer, DOPLNIT
!
...
type, extends(CVType) :: CVTypePPDIS
    ...
    contains
        procedure :: load_cv        => load_ppdis
        procedure :: calculate_cv   => calculate_ppdis
end type CVTypePPDIS
...
!===============================================================================
! Subroutine:  load_ppdis
!===============================================================================

subroutine load_ppdis(cv_item,prm_fin)
    ...
    ! read group b ----------------------------------
    write(PMF_OUT,90)
    if( .not. prmfile_get_real8_by_key(prm_fin,'pbx',cv_item%point(1)) ) then
        call pmf_utils_exit(PMF_OUT,1,'pbx is not specified')
    end if
    call pmf_unit_conv_to_ivalue(LengthUnit,cv_item%point(1))
    write(PMF_OUT,92) pmf_unit_get_rvalue(LengthUnit,cv_item%point(1)), trim(pmf_unit_label(LengthUnit))

    if( .not. prmfile_get_real8_by_key(prm_fin,'pby',cv_item%point(2)) ) then
        call pmf_utils_exit(PMF_OUT,1,'pby is not specified')
    end if
    call pmf_unit_conv_to_ivalue(LengthUnit,cv_item%point(2))
    write(PMF_OUT,94) pmf_unit_get_rvalue(LengthUnit,cv_item%point(2)), trim(pmf_unit_label(LengthUnit))

    if( .not. prmfile_get_real8_by_key(prm_fin,'pbz',cv_item%point(3)) ) then
        call pmf_utils_exit(PMF_OUT,1,'pbz is not specified')
    end if
    call pmf_unit_conv_to_ivalue(LengthUnit,cv_item%point(3))
    write(PMF_OUT,96) pmf_unit_get_rvalue(LengthUnit,cv_item%point(3)), trim(pmf_unit_label(LengthUnit))
...
96 format('   ** Z   : ',F10.3,1X,A)

end subroutine load_ppdis

!===============================================================================
! Subroutine:  calculate_ppdis
!===============================================================================

subroutine calculate_ppdis(cv_item,x,ctx)
    ...
    if( totmass1 .le. 0 ) then
        call pmf_utils_exit(PMF_OUT,1,'totmass1 is zero in calculate_ppdis!')
    end if
    d1(:) = d1(:) / totmass1
    ...
    if( info .ne. 0 ) then
    call pmf_utils_exit(PMF_OUT,1,'Unable to diagonalize matrix in calculate_ppdis!')
    end if
