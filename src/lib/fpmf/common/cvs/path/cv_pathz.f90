...
! calculate derivatives
    ...
    do i=1,cv_item%nrefs
        ...
        do  m = 1, cv_item%grps(1)
            ai = cv_item%lindexes(m)
            amass = mass(ai)
            ctx%CVsDrvs(:,ai,cv_item%idx) = ctx%CVsDrvs(:,ai,cv_item%idx) - 2.0d0*sc*sce*dx(:)*amass/(totmass1 * cv_item%alpha**2)
        end do

        do  m = cv_item%grps(i) + 1 , cv_item%grps(i+1)
            ai = cv_item%lindexes(m)
            amass = mass(ai)
            ctx%CVsDrvs(:,ai,cv_item%idx) = ctx%CVsDrvs(:,ai,cv_item%idx) + 2.0d0*sc*sce*dx(:)*amass/(totmass2 * cv_item%alpha**2)
        end do
