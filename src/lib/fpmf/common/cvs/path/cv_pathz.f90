...
type, extends(CVType) :: CVTypePATHZ
    ...
    contains
        ...
        procedure :: join_cv2path   => join_cv2path_pathz
...
! read path name  --------------------------------
    if( .not. prmfile_get_string_by_key(prm_fin,'path',cv_item%srcpathname)) then
        call pmf_utils_exit(PMF_OUT,1,'[PATHZ] The PATH name (path) is not provided!')
    end if
...
! read alpha -------------------------------------
    ...
    ! zero value will be handled in attach_cv2path_pathz()
...
!===============================================================================
! Subroutine:  join_cv2path_pathz
!===============================================================================

subroutine join_cv2path_pathz(cv_item)
...
! init algebraic indexes
    ...
    if( alloc_failed .ne. 0 ) then
        call pmf_utils_exit(PMF_OUT,1,'Unable to allocate memory for algebraicidxs and/or dsc in join_cv2path_pathz!')
    endif
    ...
    do i=1,cv_item%srcpath%ncvs
        ...
        if( cv_item%srcpath%cvs(i)%cv%gradforanycrd .neqv. .true. ) then
            call pmf_utils_exit(PMF_OUT,1,'Collective variable ''' // &
                            ...
                            ''' does not have gradforanycrd == true in join_cv2path_pathz!')
        end if
    end do
...
end subroutine join_cv2path_pathz

!===============================================================================
! Subroutine:  calculate_pathz
!===============================================================================
