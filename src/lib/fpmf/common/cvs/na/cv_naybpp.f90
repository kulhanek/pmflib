...
type, extends(CVType) :: CVTypeNAYBPP
    ...
    contains
        procedure :: load_cv        => load_naybpp
        procedure :: calculate_cv   => calculate_naybpp
end type CVTypeNAYBPP
...
!===============================================================================
! Subroutine:  load_naybpp
!===============================================================================

subroutine load_naybpp(cv_item,prm_fin)
...
end subroutine load_naybpp

!===============================================================================
! Subroutine:  calculate_naybpp
!===============================================================================

subroutine calculate_naybpp(cv_item,x,ctx)
...
! get value and derivatives ======================
    select case(cv_item%lbp_par)
        ...
        case(4)
            ! 'buckle'
            call get_vtors(ua(:,3),zsc*ub(:,3),xaxis,ctx%CVsValues(cv_item%idx))
            ! do not put zsc in get_vtors_der, it actually cancels out
            call get_vtors_der(ua(:,3),ub(:,3),xaxis,1.0d0,a_ua(:,3),a_ub(:,3),a_xaxis)
        case(5)
            ! 'propeller'
            call get_vtors(ua(:,3),zsc*ub(:,3),yaxis,ctx%CVsValues(cv_item%idx))
            ! do not put zsc in get_vtors_der, it actually cancels out
            call get_vtors_der(ua(:,3),ub(:,3),yaxis,1.0d0,a_ua(:,3),a_ub(:,3),a_yaxis)
        case(6)
            ! 'opening'
            call get_vtors(ua(:,2),zsc*ub(:,2),zaxis,ctx%CVsValues(cv_item%idx))
            ! do not put zsc in get_vtors_der, it actually cancels out
            call get_vtors_der(ua(:,2),ub(:,2),zaxis,1.0d0,a_ua(:,2),a_ub(:,2),a_zaxis)
        case default
            call pmf_utils_exit(PMF_OUT,1,'Unrecognized value for parameter option in calculate_naybppold!')
    end select
