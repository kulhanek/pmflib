...
type, extends(CVType) :: CVTypeNASBO

    type(XYZFILE_TYPE)  :: xyz_str_a
    type(XYZFILE_TYPE)  :: xyz_str_b    

    contains
        procedure :: load_cv        => load_nasbo
        procedure :: calculate_cv   => calculate_nasbo
end type CVTypeNASBO
...
end subroutine load_nasbo

!===============================================================================
! Subroutine:  calculate_nasbo
!===============================================================================

subroutine calculate_nasbo(cv_item,x,ctx)
    ...
    real(PMFDP)         :: uya(3),uyb(3),uza(3),uzb(3)
    real(PMFDP)         :: arg,tmp12,tmp22,o_tmp12,o_tmp22,o_tmp1vtmp2v,argo_tmp12,argo_tmp22 
    real(PMFDP)         :: zaxis(3),zsc,asc,f1
    real(PMFDP)         :: tmp1(3),tmp2(3),ingra,ingrb
    real(PMFDP)         :: a_tmp1(3),a_tmp2(3),a_uya(3),a_uyb(3),a_uza(3),a_uzb(3)
    real(PMFDP)         :: a_fa(4),a_fb(4),a_zaxis(3),a_rij(4,4)
    ...
    if( info .ne. 0 ) then
        call pmf_utils_exit(PMF_OUT,1,'Unable to diagonalize matrix in calculate_nasbo for point a!')
    end if
    ...
    if( info .ne. 0 ) then
        call pmf_utils_exit(PMF_OUT,1,'Unable to diagonalize matrix in calculate_nasbo for point b!')
    end if

    best = 4

    ! rotation matrix a ------------------------------
    uya(1) = 2.0d0*( fa(2,best)*fa(3,best) + fa(1,best)*fa(4,best) )
    uya(2) = fa(1,best)**2 - fa(2,best)**2 + fa(3,best)**2 - fa(4,best)**2
    uya(3) = 2.0d0*( fa(3,best)*fa(4,best) - fa(1,best)*fa(2,best) )
    ...
    ! rotation matrix b  ------------------------------
    uyb(1) = 2.0d0*( fb(2,best)*fb(3,best) + fb(1,best)*fb(4,best) )
    uyb(2) = fb(1,best)**2 - fb(2,best)**2 + fb(3,best)**2 - fb(4,best)**2
    uyb(3) = 2.0d0*( fb(3,best)*fb(4,best) - fb(1,best)*fb(2,best) )
    ...
    zsc = sign(1.0d0,uza(1)*uzb(1)+uza(2)*uzb(2)+uza(3)*uzb(3))
    ! get z-axis as average of two axes
    zaxis(:) = 0.5d0*uza(:) + 0.5d0*zsc*uzb(:)

    ! normalize - COMMENTED ON PURPOSE - it is not necessary to normalize z-axis for BO evaluation
    ! if you UNCOMMENT than gradients will not be correct
    ! zaxis(:) = zaxis(:)/sqrt(zaxis(1)**2+zaxis(2)**2+zaxis(3)**2)

    ! 'opening'
    tmp1(1) = uya(2)*zaxis(3) - uya(3)*zaxis(2)
    tmp1(2) = uya(3)*zaxis(1) - uya(1)*zaxis(3)
    tmp1(3) = uya(1)*zaxis(2) - uya(2)*zaxis(1)

    tmp2(1) = zsc*uyb(2)*zaxis(3) - zsc*uyb(3)*zaxis(2)
    tmp2(2) = zsc*uyb(3)*zaxis(1) - zsc*uyb(1)*zaxis(3)
    tmp2(3) = zsc*uyb(1)*zaxis(2) - zsc*uyb(2)*zaxis(1)
...
! with respect to uya, uyb, and zaxis
    a_uya(1) = zaxis(2)*a_tmp1(3) - a_tmp1(2)*zaxis(3) 
    a_uya(2) = zaxis(3)*a_tmp1(1) - a_tmp1(3)*zaxis(1)
    a_uya(3) = zaxis(1)*a_tmp1(2) - a_tmp1(1)*zaxis(2)

    a_uyb(1) = zsc*zaxis(2)*a_tmp2(3) - zsc*a_tmp2(2)*zaxis(3) 
    a_uyb(2) = zsc*zaxis(3)*a_tmp2(1) - zsc*a_tmp2(3)*zaxis(1)
    a_uyb(3) = zsc*zaxis(1)*a_tmp2(2) - zsc*a_tmp2(1)*zaxis(2) 

    a_zaxis(1) = a_tmp1(2)*uya(3) - uya(2)*a_tmp1(3) + zsc*a_tmp2(2)*uyb(3) - zsc*uyb(2)*a_tmp2(3) 
    a_zaxis(2) = a_tmp1(3)*uya(1) - uya(3)*a_tmp1(1) + zsc*a_tmp2(3)*uyb(1) - zsc*uyb(3)*a_tmp2(1)
...
!     uya(3) = 2.0d0*( fa(3,best)*fa(4,best) - fa(1,best)*fa(2,best) )
    ...
    a_fa(1) = 2.0d0*(fa(4,best)*a_uya(1) + fa(1,best)*a_uya(2) - fa(2,best)*a_uya(3)) 
    a_fa(2) = 2.0d0*(fa(3,best)*a_uya(1) - fa(2,best)*a_uya(2) - fa(1,best)*a_uya(3))
    a_fa(3) = 2.0d0*(fa(2,best)*a_uya(1) + fa(3,best)*a_uya(2) + fa(4,best)*a_uya(3))
    a_fa(4) = 2.0d0*(fa(1,best)*a_uya(1) - fa(4,best)*a_uya(2) + fa(3,best)*a_uya(3))

    a_fb(1) = 2.0d0*(fb(4,best)*a_uyb(1) + fb(1,best)*a_uyb(2) - fb(2,best)*a_uyb(3)) 
    a_fb(2) = 2.0d0*(fb(3,best)*a_uyb(1) - fb(2,best)*a_uyb(2) - fb(1,best)*a_uyb(3))
    a_fb(3) = 2.0d0*(fb(2,best)*a_uyb(1) + fb(3,best)*a_uyb(2) + fb(4,best)*a_uyb(3))
    a_fb(4) = 2.0d0*(fb(1,best)*a_uyb(1) - fb(4,best)*a_uyb(2) + fb(3,best)*a_uyb(3))
    
!     uza(1) = 2.0d0*( fa(2,best)*fa(4,best) - fa(1,best)*fa(3,best) )
!     uza(2) = 2.0d0*( fa(3,best)*fa(4,best) + fa(1,best)*fa(2,best) )
!     uza(3) = fa(1,best)**2 - fa(2,best)**2 - fa(3,best)**2 + fa(4,best)**2

    a_fa(1) = a_fa(1) + 2.0d0*(-fa(3,best)*a_uza(1) + fa(2,best)*a_uza(2) + fa(1,best)*a_uza(3)) 
    a_fa(2) = a_fa(2) + 2.0d0*( fa(4,best)*a_uza(1) + fa(1,best)*a_uza(2) - fa(2,best)*a_uza(3))
    a_fa(3) = a_fa(3) + 2.0d0*(-fa(1,best)*a_uza(1) + fa(4,best)*a_uza(2) - fa(3,best)*a_uza(3))
    a_fa(4) = a_fa(4) + 2.0d0*( fa(2,best)*a_uza(1) + fa(3,best)*a_uza(2) + fa(4,best)*a_uza(3))

    a_fb(1) = a_fb(1) + 2.0d0*(-fb(3,best)*a_uzb(1) + fb(2,best)*a_uzb(2) + fb(1,best)*a_uzb(3)) 
    a_fb(2) = a_fb(2) + 2.0d0*( fb(4,best)*a_uzb(1) + fb(1,best)*a_uzb(2) - fb(2,best)*a_uzb(3))
    a_fb(3) = a_fb(3) + 2.0d0*(-fb(1,best)*a_uzb(1) + fb(4,best)*a_uzb(2) - fb(3,best)*a_uzb(3))
    a_fb(4) = a_fb(4) + 2.0d0*( fb(2,best)*a_uzb(1) + fb(3,best)*a_uzb(2) + fb(4,best)*a_uzb(3))
...
end subroutine calculate_nasbo
