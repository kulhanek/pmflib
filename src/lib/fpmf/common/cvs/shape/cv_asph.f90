...
! Principal Moments Of Gyration Tensor - asphericity b - ASPH
...
type, extends(CVType) :: CVTypeASPH
    contains
        procedure :: load_cv        => load_asph
        procedure :: calculate_cv   => calculate_asph
end type CVTypeASPH
...
end subroutine load_asph

!===============================================================================
! Subroutine:  calculate_asph
!===============================================================================

subroutine calculate_asph(cv_item,x,ctx)
    ...
    if( info .ne. 0 ) then
        call pmf_utils_exit(PMF_OUT,1,'Unable to diagonalize T matrix in calculate_asph!')
    end if

    ctx%CVsValues(cv_item%idx) = eigenvalues(3) - 0.5d0*( eigenvalues(1) + eigenvalues(2) )
