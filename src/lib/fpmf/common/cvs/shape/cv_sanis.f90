...
! Principal Moments Of Gyration Tensor - relative shape anisotropy κ2 SANIS
...
type, extends(CVType) :: CVTypeSANIS
    contains
        procedure :: load_cv        => load_sanis
        procedure :: calculate_cv   => calculate_sanis
end type CVTypeSANIS
...
!===============================================================================
! Subroutine:  load_sanis
!===============================================================================

subroutine load_sanis(cv_item,prm_fin)
...
end subroutine load_sanis

!===============================================================================
! Subroutine:  calculate_sanis
!===============================================================================

subroutine calculate_sanis(cv_item,x,ctx)
    ...
    ! -----------------------------------------------
    integer        :: ai,m,info
    real(PMFDP)    :: com(3),amass,totmass,itotmass,fac,dv1,dv2,dv3,rg2,b,c
    ...
    if( info .ne. 0 ) then
        call pmf_utils_exit(PMF_OUT,1,'Unable to diagonalize T matrix in calculate_sanis!')
    end if

    rg2 = eigenvalues(1) + eigenvalues(2) + eigenvalues(3)
    b   = eigenvalues(3) - 0.5d0 * (eigenvalues(1) + eigenvalues(2))
    c   = eigenvalues(2) - eigenvalues(1)

    ctx%CVsValues(cv_item%idx) = ( b**2 + 0.75d0 * c**2 )/ rg2**2

    ! calculate derivatives -------------------------

    dv1 = (    - b - 1.5d0*c) / rg2**2 - ( b**2 + 0.75d0 * c**2 ) / rg2**4 * 2.0d0 * rg2
    dv2 = (    - b + 1.5d0*c) / rg2**2 - ( b**2 + 0.75d0 * c**2 ) / rg2**4 * 2.0d0 * rg2
    dv3 = (2.0d0*b          ) / rg2**2 - ( b**2 + 0.75d0 * c**2 ) / rg2**4 * 2.0d0 * rg2

    do m = 1, cv_item%natoms
        ai = cv_item%lindexes(m)
        amass = mass(ai)
        fac = 2.0d0 * dv1 * amass * itotmass
        ...
        fac = 2.0d0 * dv3 * amass * itotmass
        ctx%CVsDrvs(1,ai,cv_item%idx) = ctx%CVsDrvs(1,ai,cv_item%idx) + fac*((x(1,ai) - com(1))*t(1,3)*t(1,3) + &
                                   (x(2,ai) - com(2))*t(1,3)*t(2,3) + &
                                   (x(3,ai) - com(3))*t(1,3)*t(3,3) )

        ctx%CVsDrvs(2,ai,cv_item%idx) = ctx%CVsDrvs(2,ai,cv_item%idx) + fac*((x(1,ai) - com(1))*t(2,3)*t(1,3) + &
                                   (x(2,ai) - com(2))*t(2,3)*t(2,3) + &
                                   (x(3,ai) - com(3))*t(2,3)*t(3,3) )

        ctx%CVsDrvs(3,ai,cv_item%idx) = ctx%CVsDrvs(3,ai,cv_item%idx) + fac*((x(1,ai) - com(1))*t(3,3)*t(1,3) + &
                                   (x(2,ai) - com(2))*t(3,3)*t(2,3) + &
                                   (x(3,ai) - com(3))*t(3,3)*t(3,3) )
    end do

    return

end subroutine calculate_sanis
