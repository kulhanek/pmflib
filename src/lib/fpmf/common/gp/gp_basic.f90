...
subroutine gp_basic_teach(self, len_scale, periodicity, f_var, kernel, f_r, f_v, f_n, g_r, g_v, g_n, &
                          f_pseudo_sparse_set, g_pseudo_sparse_set, jitter, partial, sparsemethod, verbose)
   ...
   if (self%sparsified) then
      ...
      if (trim(self%sparsemethod) .eq. 'fitc') then
          ...
          do i=1, n_f
             call Matrix_QR_Solve(self%Kmm, Kmn(:,i), Kmm_inv_kn)
             Lnn(i) = self%f_var - ddot(self%m_teach, Kmm_inv_kn(:), 1, Kmn(:,i), 1)
          end do
          do i=1, n_g
             do ii=1, self%n_dof
                call Matrix_QR_Solve(self%Kmm, Kmn(:,n_f+(i-1)*self%n_dof+ii), Kmm_inv_kn)
                Lnn(n_f+(i-1)*self%n_dof+ii) = self%f_var / self%len_scale_sq(ii) &
      ...
      if (trim(self%sparsemethod) .eq. 'fitc') then
          do i=1, n_f
             siginvsq_y(i) = f_v(i)/(Lnn(i)+f_n(i))
          end do
          do i=1, n_g
             siginvsq_y(n_f+(i-1)*self%n_dof+1:n_f+i*self%n_dof) = &
                                                       g_v(:,i)/(Lnn(n_f+(i-1)*self%n_dof+1:n_f+i*self%n_dof)+g_n(:,i))
          end do
      else
          do i=1, n_f
             y(i) = f_v(i)
      ...
      if (allocated(self%Kmn_siginvsq_y)) then
          call dgemv('N', size(Kmn,1), size(Kmn,2), 1.0_dp, Kmn, size(Kmn,1), &
                     siginvsq_y, 1, 1.0_dp, self%Kmn_siginvsq_y, 1)
      else
      ...
      if (trim(self%sparsemethod) .eq. 'fitc') then
          do i=1, n_f
             siginvsq_Knm(i,:) = siginvsq_Knm(i,:) / (Lnn(i)+f_n(i))
          end do
          do i=1, n_g
             do ii=1, self%n_dof
                siginvsq_Knm(n_f+(i-1)*self%n_dof+ii,:) = &
                                        siginvsq_Knm(n_f+(i-1)*self%n_dof+ii,:) / (Lnn(n_f+(i-1)*self%n_dof+ii)+g_n(ii,i))
             end do
          end do
      else
          do i=1, n_f
      ...
      if (allocated(self%Kmn_siginvsq_Knm)) then
          call dgemm('N', 'N', size(Kmn,1), size(siginvsq_Knm,2), size(Kmn,2), &
                     ...
                     1.0_dp, self%Kmn_siginvsq_Knm, size(self%Kmn_siginvsq_Knm,1))
      else
...
subroutine gp_basic_write(self,fileid,filename,mode)
   ...
   ! partial mode
      ...
      if(self%sparsified) then
         ...
         write(fileid,*) self%Kmn_siginvsq_y
         write(fileid,*) '# self%Kmn_siginvsq_Knm(self%m_teach,self%m_teach)'
         write(fileid,*) self%Kmn_siginvsq_Knm
      else
      end if
   end if
   ! full mode
   if(trim(do_mode) .eq. 'full') then
      if(self%sparsified) then
         ...
         write(fileid,*) self%noise_Kmm%matrix
         write(fileid,*) '# self%logL'
         write(fileid,*) self%logL
      end if
   end if
...
subroutine gp_basic_read(self, fileid, filename)
   ...
   read(fileid,*)
   allocate(self%len_scale_sq(self%n_dof))
   read(fileid,*) self%len_scale_sq
   read(fileid,*)
   allocate(self%periodicity(self%n_dof))
   read(fileid,*) self%periodicity
   if(self%m_f .ne. 0) then
      allocate(self%f_r(self%n_dof,self%m_f))
      read(fileid,*)
      read(fileid,*) self%f_r
   endif
   if(self%m_g .ne. 0) then
      allocate(self%g_r(self%n_dof,self%m_g))
      read(fileid,*)
      read(fileid,*) self%g_r
   endif
   ! compact mode
   if(trim(self%mode) .eq. 'compact') then
      allocate(self%Cmat_inv_v(self%m_teach))
      read(fileid,*)
      read(fileid,*) self%Cmat_inv_v
   ...
   ! partial mode
      ...
      if(self%sparsified) then
         ...
         read(fileid,*)
         read(fileid,*) self%jitter
         read(fileid,*)
         read(fileid,*) self%y_siginvsq_y
         allocate(self%Kmn_siginvsq_y(self%m_teach))
         read(fileid,*)
         read(fileid,*) self%Kmn_siginvsq_y
         allocate(self%Kmn_siginvsq_Knm(self%m_teach,self%m_teach))
         read(fileid,*)
         read(fileid,*) self%Kmn_siginvsq_Knm
      else
      end if
   end if
   ! full mode
...
subroutine SE_kernel_r_rr(x1, x2, f_var, len_scale_sq, periodicity, f_f, g_f, f_g, g_g)
   ...
   do i=1, n_dof
      if (periodicity(i) > 0.0_dp) then
         pi_per_periodicity(i) = TWO_PI/periodicity(i)/2.0_dp
         two_pi_per_periodicity(i) = TWO_PI/periodicity(i)
      else
   ...
   do i=1, n_dof
      diff(:) = x2(i,:)-x1(i)
      if (periodicity(i) > 0.0_dp) then
         exp_arg(:) = exp_arg(:) - 2.0_dp*sin(pi_per_periodicity(i)*diff(:))**2/len_scale_sq(i)
      else if (periodicity(i) < 0.0_dp) then
         diff(:) = diff(:) + nint(-diff(:)/periodicity(i))*periodicity(i)
         exp_arg(:) = exp_arg(:) - 0.5_dp*diff(:)**2/len_scale_sq(i)
      else
         exp_arg(:) = exp_arg(:) - 0.5_dp*diff(:)**2/len_scale_sq(i)
      endif
   end do
      ...
      do i=1, n_dof
         ...
         if (periodicity(i) > 0.0_dp) then
            dexp_arg_i(i,:) = 2.0_dp*sin(pi_per_periodicity(i)*diff(:)) &
                            * cos(pi_per_periodicity(i)*diff(:))*two_pi_per_periodicity(i) / len_scale_sq(i)
            if (present(g_g)) ddexp_arg_i(i,:) = (-sin(pi_per_periodicity(i)*diff(:))**2 &
                            + cos(pi_per_periodicity(i)*diff(:))**2)*two_pi_per_periodicity(i)**2/len_scale_sq(i)
         else if (periodicity(i) < 0.0_dp) then
            diff(:) = diff(:) + nint(-diff(:)/periodicity(i))*periodicity(i)
            dexp_arg_i(i,:) = diff(:)/len_scale_sq(i)
      ...
      do j=1, n_dof
         ...
         if (periodicity(j) > 0.0_dp) then
            dexp_arg_j(j,:) = -2.0_dp*sin(pi_per_periodicity(j)*diff(:)) & 
                            * cos(pi_per_periodicity(j)*diff(:))*two_pi_per_periodicity(j) / len_scale_sq(j)
         else if (periodicity(j) < 0.0_dp) then
            diff(:) = diff(:) + nint(-diff(:)/periodicity(j))*periodicity(j)
            dexp_arg_j(j,:) = -diff(:)/len_scale_sq(j)
         else
            dexp_arg_j(j,:) = -diff(:)/len_scale_sq(j)
         endif
      end do
