            ...
            switch(FDLevel) {
                case 3:
                    ...
                    if(! trial) {
                        ...
                        Rhs[LocIter] = Accumulator->GetValue(ifcoord,ifbin1,EABF_DG_VALUE);
                        LocIter++;
                        A[LocIter][XMap[ifbin1]] = -1.0 * dfac;
                        A[LocIter][XMap[ifbin3]] = +1.0 * dfac;
                        Rhs[LocIter] = Accumulator->GetValue(ifcoord,ifbin2,EABF_DG_VALUE);
                        LocIter++;
                        A[LocIter][XMap[ifbin1]] = +1.0 * dfac;
                        A[LocIter][XMap[ifbin2]] = -4.0 * dfac;
                        A[LocIter][XMap[ifbin3]] = +3.0 * dfac;
                        Rhs[LocIter] = Accumulator->GetValue(ifcoord,ifbin3,EABF_DG_VALUE);
                        LocIter++;
                    } else {
                        NumOfEquations += 3;
                    }
