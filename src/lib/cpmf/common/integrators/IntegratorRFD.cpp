            ...
            switch(FDLevel) {
                case 3:
                    ...
                    if(trial == false) {
                        ...
                        Rhs[LocIter] = DerProxy->GetValue(ifbin1,ifcoord,E_PROXY_MEAN)*rfac;
                        RhsCv[LocIter] = ifcoord;
                        LocIter++;
                        cs_entry(A,LocIter,XMap[ifbin1],-1.0*lfac);
                        cs_entry(A,LocIter,XMap[ifbin3],+1.0*lfac);
                        Rhs[LocIter] = DerProxy->GetValue(ifbin2,ifcoord,E_PROXY_MEAN)*rfac;
                        RhsCv[LocIter] = ifcoord;
                        LocIter++;
                        cs_entry(A,LocIter,XMap[ifbin1],+1.0*lfac);
                        cs_entry(A,LocIter,XMap[ifbin2],-4.0*lfac);
                        cs_entry(A,LocIter,XMap[ifbin3],+3.0*lfac);
                        Rhs[LocIter] = DerProxy->GetValue(ifbin3,ifcoord,E_PROXY_MEAN)*rfac;
                        RhsCv[LocIter] = ifcoord;
                        LocIter++;
                    } else {
                        NumOfEquations += 3;
                ...
                case 4:
                    ...
                    if(trial == false) {
                        ...
                        Rhs[LocIter] = DerProxy->GetValue(ifbin4,ifcoord,E_PROXY_MEAN);
                        RhsCv[LocIter] = ifcoord;
                        LocIter++;
                    } else {
                        NumOfEquations += 4;
